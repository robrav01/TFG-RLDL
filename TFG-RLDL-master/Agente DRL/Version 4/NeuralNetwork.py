...
class DQN:
    def __init__(self, acciones, learning_rate, discount_factor, exploration):
        ...
        self.model.add(Dense(17, input_dim=17, activation='relu'))
        ...
        for i in range(8):
            for j in range(8):
                print(self.model.predict(np.array([Utils.one_hot([i, j, 0])])))
                print(np.argmax(self.model.predict(np.array([Utils.one_hot([i, j, 0])]))))
    ...
    def take_action(self, state):
        if (random.uniform(0, 1) < self.exploration):    # Accion aleatoria
            ...
            Q = self.model.predict(np.array([Utils.one_hot(state)]))
            while True:
                action = np.argmax(Q)
                nextState = self.calculaNextState(state, action)
    ...
    def learn(self,observations):
        ...
        for i in range(0, len(batch)):
            state = Utils.one_hot(batch[i][0])
            action = batch[i][1]
            state_new = Utils.one_hot(batch[i][2])
            reward = batch[i][3]
            done = batch[i][4]
