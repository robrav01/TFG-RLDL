...
class Maze(tkinter.Tk, object):
   ...
   def __init__(self, width, height, coin, end_state):
       super(Maze, self).__init__()
       self.width = width
       self.height = height
       self.escape = end_state
   ...
   def _create_escape(self):
       #self.escape = self.grid[(self.width * self.height)-1]    # Sustituir width y height por la posicion de la salida
       self.escape = self.grid[self.escape[0] + (self.escape[1]*self.height)]
